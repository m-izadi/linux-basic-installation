...
######## Proxy
echo -e "\nWould you like Set Proxy? (Default No) (Y/n) "
read -r proxy
...
# fi
if [ "$proxy" == "Y" ] ; then
    echo "Please enter your Proxy: "
    read -r proxy_address
	proxy_address="${proxy_address^^}" #toUpperCase

fi
#Remove Proxy ()
            # echo -e "\nWould you like Remove Proxy after Installation? (Default No) (Y/n) "
            # read -r rm_proxy
            # proxy="${rm_proxy^^}" #toUpperCase
            # # if [ -z "$proxy" ]; then
            # # 	proxy="N"
            # # fi
            # if [ "$rm_proxy" == "Y" ] ; then
